...
(function (root) {
  ...
  function buildNavbar() {
    ...
    PRIMARY.concat([UTILITY[1]]).forEach((e) => els.navbar.appendChild(navItem(e, "navbar__item")));
  }
  ...
  function updateNav() {
    ...
    const equivalent = w === "capture" ? "manage" : w;
  ...
  function buildSearch() {
    ...
    document.addEventListener("pointerdown", (e) => {
      if (searchOpen && !wrap.contains(e.target)) closeSearch();
    });
  ...
  function updateActions() {
    ...
    els.actions.appendChild(captureChip());
  }

  function captureChip() {
    const info = root.XBCapture.summarize(St.state.capture);
    const chip = h("button.capture", {
      type: "button",
      "data-status": info.status,
      "aria-label": "Capture status: " + info.label,
      title: "Capture status",
    });
    chip.appendChild(h("span.capture__dot", { "aria-hidden": "true" }));
    chip.appendChild(h("span", { text: info.label }));
    chip.addEventListener("click", () => go("capture"));
    return chip;
  }
  ...
  function updateBulk() {
    ...
    bar.appendChild(button("ctl", {
      icon: "seen", label: "Mark seen", on: () => { St.setSeen(ids, true); toast(num(ids.length) + " marked as seen"); },
    }));
    bar.appendChild(button("ctl", {
      icon: allArchived ? "unarchive" : "archive",
      label: allArchived ? "Unarchive" : "Archive",
      on: () => { St.setArchived(ids, !allArchived); toast(allArchived ? "Unarchived" : "Archived"); },
    }));
  ...
  function bindGlobalKeys() {
    document.addEventListener("keydown", (e) => {
      ...
      if (k === "d" || k === "D") go("discover");
        ...
        go("library");
        const btn = els.stage.querySelector(".libbar__right .ctl:nth-child(2)");
        if (btn) btn.click();
      } else if (k === "?") {
        go("settings");
  ...
  function loadSample() {
    if (!root.XB_DEMO || !root.XB_DEMO.bookmarks) { toast("Sample library unavailable", { error: true }); return; }
