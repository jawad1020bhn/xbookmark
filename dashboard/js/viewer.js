...
(function (root) {
  ...
  function build() {
    ...
    const chrome = h(".viewer__chrome", top, foot, strip);
  ...
  function paintPhoto(item) {
    ...
    /* Zoom: double-click steps, ctrl+wheel is continuous. Both write the same
       custom property so there is one source of truth for scale. */
    let zoom = 1;
    const apply = (z) => {
    ...
    refs.frame.addEventListener("wheel", (e) => {
  ...
  function paintVideo(item) {
    const video = root.M3EMedia.createVideo(item.media, {
      ...
      autoplay: !(root.M3E && root.M3E.reducedMotion && root.M3E.reducedMotion()),
      preload: "auto",
